...
def get_datas(train_env_dirs, env_idx, dataset_idx):
    ...
    for i, data in enumerate(datas):
        ...
        sample_pos = utils.normalize_state(sample_pos)
...
def collect_mistakes(dataset_list, colchecker, iter_num, balance=False):
    ...
    for i, data in enumerate(dataset_list):
        occ_grid, start_pos, goal_pos, sample_pos, col_label, sel_label = data
        ...
        with torch.no_grad():
            ...
            samples_t = torch.tensor(sample_pos, device=device, dtype=torch.float)
        ...
        print(len(col_scores), len(col_label), len(sample_pos))
        for i, score in enumerate(col_scores):
            if col_label[i] == 1:
                if score < COL_PRED_THRESHOLD:
                    col_false_neg_dataset.append([occ_grid, start_pos, goal_pos, sample_pos[i], col_label[i], sel_label[i]])
                else:
                    col_true_pos_dataset.append([occ_grid, start_pos, goal_pos, sample_pos[i], col_label[i], sel_label[i]])

            elif col_label[i] == 0:
                if score > COL_PRED_THRESHOLD:
                    col_false_pos_dataset.append([occ_grid, start_pos, goal_pos, sample_pos[i], col_label[i], sel_label[i]])
                else:
                    col_true_neg_dataset.append([occ_grid, start_pos, goal_pos, sample_pos[i], col_label[i], sel_label[i]])
...
if __name__ == '__main__':
    ...
    linkpos_dim = 12
    occ_grid_dim = 100
    model_name = "model_col_dagger_global"
    ...
    # env dirs
    train_env_dirs = osp.join(CUR_DIR, "dataset/model_global")
    eval_data_dir = osp.join(CUR_DIR, "./dataset/model_eval_global")
