...
if __name__ == '__main__':
    # constants
    model_name = "model"
    data_dir = osp.join(CUR_DIR, "./dataset/{}_global".format(model_name))
    data_dir_t = osp.join(CUR_DIR, "./dataset/{}_global_t".format(model_name))

    col_train_data_dir = osp.join(CUR_DIR, "./dataset/{}_col_global".format(model_name))
    if not os.path.exists(col_train_data_dir):
        os.makedirs(col_train_data_dir)
    sel_train_data_dir = osp.join(CUR_DIR, "./dataset/{}_sel_global".format(model_name))
    ...
    col_eval_data_dir = osp.join(CUR_DIR, "./dataset/{}_col_eval_global".format(model_name))
    if not os.path.exists(col_eval_data_dir):
        os.makedirs(col_eval_data_dir)
    sel_eval_data_dir = osp.join(CUR_DIR, "./dataset/{}_sel_eval_global".format(model_name))
    if not os.path.exists(sel_eval_data_dir):
        os.makedirs(sel_eval_data_dir)
...
#         col_pos_samples = col_pos_samples[:len(col_neg_samples)]
    if len(col_pos_samples) > 55000:
        eval_col_data += col_pos_samples[50000:55000]
        col_pos_samples = col_pos_samples[:50000]
    if len(col_neg_samples) > 55000:
        eval_col_data += col_neg_samples[50000:55000]
        col_neg_samples = col_neg_samples[:50000]
    print(len(col_pos_samples), len(col_neg_samples), len(eval_col_data))
    ...
    if len(sel_pos_samples) > 55000:
        eval_sel_data += sel_pos_samples[50000:55000]
        sel_pos_samples = sel_pos_samples[:50000]
    if len(sel_neg_samples) > 55000:
        eval_sel_data += sel_neg_samples[50000:55000]
        sel_neg_samples = sel_neg_samples[:50000]
    print(len(sel_pos_samples), len(sel_neg_samples), len(eval_sel_data))
...
#     # print("integrated_train_data_cnt", integrated_train_data_cnt)
    ...
    for sample in eval_col_data:
        new_file_path = osp.join(col_eval_data_dir, "data_{}.pkl".format(col_eval_data_cnt))
        with open(new_file_path, 'wb') as f:
            # print("Dumping to {}".format(file_path))
            pickle.dump(sample, f)
        col_eval_data_cnt += 1

    for sample in eval_sel_data:
        new_file_path = osp.join(sel_eval_data_dir, "data_{}.pkl".format(sel_eval_data_cnt))
        with open(new_file_path, 'wb') as f:
            # print("Dumping to {}".format(file_path))
            pickle.dump(sample, f)
        sel_eval_data_cnt += 1

    # pos_samples, neg_samples, col_samples = get_datas_eval(data_dir, 25, 500, 10)
    ...
    # random.shuffle(col_pos_samples)
    # random.shuffle(col_neg_samples)
    # print(len(col_pos_samples), len(neg_samples))
    # all_data = col_pos_samples + col_neg_samples
    # for sample in all_data:
