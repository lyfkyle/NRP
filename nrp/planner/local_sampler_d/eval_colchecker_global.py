...
from model import ColCheckerSmall
import utils
...
if __name__ == '__main__':
    ...
    model = ColCheckerSmall(robot_dim + linkpos_dim, occ_grid_size=occ_grid_dim)
    ...
    for data in dataloader:
        ...
        with torch.no_grad():
            col_scores = model(occ_grid, start, sample, fixed_env=False)
            # sel_scores = sel_scores.view(occ_grid.shape[0], -1)

        # calculate loss
        # loss, _ = contrastive_loss(sel_scores, pos_idx
        loss = bce_loss(col_scores, label)
    ...
    print('col_pred: %.3f' % (eval_total_loss))
    ...
    disp.plot()
    plt.savefig("col_confusion_matrix_global.png")