...
def collect_mistakes(dataset_list, selector, iter_num, balance=False):
    ...
    for i, data in enumerate(dataset_list):
        ...
        if len(col_false_pos_dataset) >= 50:
            col_neg_dataset = col_false_pos_dataset[:50]
        else:
            col_neg_dataset = col_false_pos_dataset

        # maximum 50 positive data
        if len(col_false_neg_dataset) >= 50:
            col_pos_dataset = col_false_neg_dataset[:50]
        else:
            col_pos_dataset = col_false_neg_dataset
        ...
        # maximum 50 negative data
        if len(sel_false_pos_dataset) >= 50:
            sel_neg_dataset = sel_false_pos_dataset[:50]
        else:
            sel_neg_dataset = sel_false_pos_dataset

        # maximum 50 positive data
        if len(sel_false_neg_dataset) >= 50:
            sel_pos_dataset = sel_false_neg_dataset[:50]
        else:
            sel_pos_dataset = sel_false_neg_dataset

        neg_dataset = col_neg_dataset + sel_neg_dataset
        pos_dataset = col_pos_dataset + sel_pos_dataset
        true_pos_dataset = col_true_pos_dataset + sel_true_pos_dataset
        true_neg_dataset = col_true_neg_dataset + sel_true_neg_dataset
...
def collect_data_from_train_env(train_env_dirs, data_cnt, iter_num, dataset_list):
    ...
    selector = SelectModel(robot_dim, occ_grid_dim)
...
def train(data_cnt, epoch_num, iter_num, train_num, best_loss):
    ...
    for _ in range(epoch_num):
        ...
        preds_binary = (np.array(col_all_preds) > SEL_PRED_THRESHOLD).astype(int)
        ...
        for label in col_all_labels:
            if label == 1:
                ...
                num_neg += 1
        writer.add_scalar('dataset_col_num_pos/iter', num_pos, train_num)
        writer.add_scalar('dataset_col_num_neg/iter', num_neg, train_num)
        ...
        for label in sel_all_labels:
            if label == 1:
                ...
                num_neg += 1
        writer.add_scalar('dataset_sel_num_pos/iter', num_pos, train_num)
        writer.add_scalar('dataset_sel_num_neg/iter', num_neg, train_num)
...
if __name__ == '__main__':
    ...
    linkpos_dim = 12
    occ_grid_dim = 40
    model_name = "model_integrated_dagger"
    ...
    num_epochs = 40
    max_iter = 100
    target_data_cnt = 1079820
    max_data_cnt = 1200000
    epoch_num_1 = 50
    epoch_num_2 = 10
    epoch_num_3 = 50
    first_time = False
    train_data_step = 20000
    data_cnt = 1079820
    ...
    dataset = MyDataset(data_cnt, None, None)
    dataloader = DataLoader(dataset, batch_size=bs, shuffle=True, drop_last=True, num_workers=40, pin_memory=True)
    for _ in range(epoch_num_3):
        dataset_sel_accuracy = 0
        for data in dataloader:
            occ_grid, start, goal, sample, col_label, sel_label = data
