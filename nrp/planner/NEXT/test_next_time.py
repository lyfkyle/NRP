...
# Constants
env_dir = osp.join(CUR_DIR, f"../env/dataset/{args.env}/test_env")
model_path = osp.join(CUR_DIR, "trained_models/NEXT_3d.pt")
# best_model_path = osp.join(CUR_DIR, "models/next_v2_best.pt")
res_dir = osp.join(CUR_DIR, f"../test/eval_res/{args.env}/test_time/{args.name}")
...
env = MyMazeEnv(args.env, robot_dim, env_dir, test=True)
model = Model(env, cuda=cuda, dim=robot_dim, env_width=occ_grid_dim)
mse_loss = torch.nn.MSELoss()

# if args.checkpoint != "":
print("Loading checkpoint {}.pt".format(model_path))
