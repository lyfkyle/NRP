...
class MyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        with open(file_path, 'rb') as f:
            data = pickle.load(f)
        occ_grid, start, sample, label = data
        ...
        start_linkpos = self.fk.get_link_positions(start_t.view(1, -1)).view(-1)
        start_t = torch.cat((start_t, start_linkpos), dim=-1)
        sample_linkpos = self.fk.get_link_positions(sample_t.view(1, -1)).view(-1)
        sample_t = torch.cat((sample_t, sample_linkpos), dim=-1)
...
if __name__ == '__main__':
    ...
    model_path = osp.join(CUR_DIR, "weights/{}.pt".format(model_name))
    data_dir = osp.join(CUR_DIR, "./dataset/{}_train".format(model_name))
    ...
    if args.checkpoint != '':
        ...
        model.load_state_dict(torch.load(osp.join(CUR_DIR, 'weights/{}.pt'.format(args.checkpoint))))
    ...
    for iter_num in range(num_epochs):
        ...
        for data in dataloader:
            ...
            # col_scores = model_parallel(occ_grid_t, v1_t, v2_t)
            col_scores = model(occ_grid_t, v1_t, v2_t)
            ...
            if loss.item() < best_loss:
                best_loss = loss.item()
                torch.save(model.state_dict(), osp.join(CUR_DIR, "weights/{}_best.pt".format(model_name)))
