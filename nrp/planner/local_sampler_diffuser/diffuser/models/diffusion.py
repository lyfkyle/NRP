...
class GaussianDiffusion(nn.Module):
    def __init__(self, model, horizon, observation_dim, action_dim, n_timesteps=1000,
        ...
        condition_guidance_w=0.1, inference=False):
        super().__init__()
    ...
    def p_mean_variance(self, x, cond, t, occ_grid, goal, collision, compute_embedding=True):
        if self.model.calc_energy:
            assert self.predict_epsilon
            x = torch.tensor(x, requires_grad=True)
            t = torch.tensor(t, dtype=torch.float, requires_grad=True)
            occ_grid.requires_grad = True
            goal.requires_grad = True
            collision.requires_grad = True

        if self.returns_condition:
            # epsilon could be epsilon or x0 itself
            epsilon_cond = self.model(x, cond, t, occ_grid, goal, collision, get_embedding=compute_embedding, use_dropout=False)
            epsilon_uncond = self.model(x, cond, t, occ_grid, goal, collision, get_embedding=compute_embedding, force_dropout=True)
            epsilon = epsilon_uncond + self.condition_guidance_w*(epsilon_cond - epsilon_uncond)
        else:
            epsilon = self.model(x, cond, t, occ_grid, goal, collision, get_embedding=compute_embedding)
    ...
    @torch.no_grad()
    def p_sample(self, x, cond, t, occ_grid, goal, collision, compute_embedding=True):
        b, *_, device = *x.shape, x.device
        model_mean, _, model_log_variance = self.p_mean_variance(x=x, cond=cond, t=t, occ_grid=occ_grid, goal=goal, collision=collision, compute_embedding=compute_embedding)
    ...
    @torch.no_grad()
    def p_sample_loop(self, shape, cond, occ_grid, goal, collision, verbose=True, return_diffusion=False):
        ...
        for i in reversed(range(0, self.n_timesteps)):
            ...
            if self.inference and (i < self.n_timesteps - 1):
                x = self.p_sample(x, cond, timesteps, occ_grid, goal, collision, compute_embedding=False)
            else:
                x = self.p_sample(x, cond, timesteps, occ_grid, goal, collision)
            x = apply_conditioning(x, cond, self.action_dim)
    ...
    @torch.no_grad()
    def conditional_sample(self, cond, occ_grid, goal, collision, horizon=None, *args, **kwargs):
        '''
        ...
        return self.p_sample_loop(shape, cond, occ_grid, goal, collision, *args, **kwargs)

    def grad_p_sample(self, x, cond, t, occ_grid, goal, collision):
        b, *_, device = *x.shape, x.device
        model_mean, _, model_log_variance = self.p_mean_variance(x=x, cond=cond, t=t, occ_grid=occ_grid, goal=goal, collision=collision)
        noise = 0.5*torch.randn_like(x)
        # no noise when t == 0
        nonzero_mask = (1 - (t == 0).float()).reshape(b, *((1,) * (len(x.shape) - 1)))
        return model_mean + nonzero_mask * (0.5 * model_log_variance).exp() * noise

    def grad_p_sample_loop(self, shape, cond, occ_grid, goal, collision, verbose=True, return_diffusion=False):
        device = self.betas.device
        ...
        for i in reversed(range(0, self.n_timesteps)):
            timesteps = torch.full((batch_size,), i, device=device, dtype=torch.long)
            x = self.grad_p_sample(x, cond, timesteps, occ_grid, goal, collision)
            x = apply_conditioning(x, cond, self.action_dim)
    ...
    def grad_conditional_sample(self, cond, occ_grid, goal, collision, horizon=None, *args, **kwargs):
        '''
            conditions : [ (time, state), ... ]
        '''
        device = self.betas.device
        ...
        return self.grad_p_sample_loop(shape, cond, occ_grid, goal, collision, *args, **kwargs)
    ...
    def p_losses(self, x_start, cond, t, occ_grid, goal, collision):
        noise = torch.randn_like(x_start)
        ...
        x_recon = self.model(x_noisy, cond, t, occ_grid, goal, collision)
    ...
    def loss(self, x, cond, occ_grid, goal, collision):
        batch_size = len(x)
        t = torch.randint(0, self.n_timesteps, (batch_size,), device=x.device).long()
        return self.p_losses(x, cond, t, occ_grid, goal, collision)

    def forward(self, cond, occ_grid, goal, collision, *args, **kwargs):
        return self.conditional_sample(cond=cond, occ_grid=occ_grid, goal=goal, collision=collision, *args, **kwargs)
