...
from local_sampler_g.model import CNN_Encoder
...
class TemporalUnet(nn.Module):
    ...
    def __init__(
        ...
        self.cnn_encoder = CNN_Encoder(output_size=dim, input_size=(1, 40, 40))
        self.goal_mlp = nn.Sequential(
            nn.Linear(8 + 1, dim),  # goal pos + base orientation
            act_fn,
            nn.Linear(dim, dim * 4),
            act_fn,
            nn.Linear(dim * 4, dim),
        ...
        self.collision_mlp = nn.Sequential(
            nn.Linear(collision_dim, dim),  # nn.Linear(horizon, dim),
            act_fn,
            nn.Linear(dim, dim * 4),
            act_fn,
            nn.Linear(dim * 4, dim),
        ...
        self.mask_dist = Bernoulli(probs=1-self.condition_dropout)
        embed_dim = 3 * dim + 512  # TODO: generalize this. Currently hardcoding encoder output dim
    ...
    def forward(self, x, cond, time, occ_grid, goal, collision, get_embedding=True, use_dropout=True, force_dropout=False):
        '''
        ...
        if get_embedding:
            self.occ_grid_embed = self.cnn_encoder(occ_grid)
            self.goal_embed = self.goal_mlp(goal)
            self.collision_embed = self.collision_mlp(collision)
            # print("getting embedding")
        occ_grid_embed = self.occ_grid_embed
        goal_embed = self.goal_embed
        collision_embed = self.collision_embed
        if use_dropout:
            mask = self.mask_dist.sample(sample_shape=(self.occ_grid_embed.size(0), 1)).to(self.occ_grid_embed.device)
            occ_grid_embed = mask * self.occ_grid_embed
            mask = self.mask_dist.sample(sample_shape=(self.goal_embed.size(0), 1)).to(self.goal_embed.device)
            goal_embed = mask * self.goal_embed
            mask = self.mask_dist.sample(sample_shape=(self.collision_embed.size(0), 1)).to(self.collision_embed.device)
            collision_embed = mask * self.collision_embed
        if force_dropout:
            occ_grid_embed = 0 * self.occ_grid_embed
            goal_embed = 0 * self.goal_embed
            collision_embed = 0 * self.collision_embed

        t = torch.cat([t, occ_grid_embed, goal_embed, collision_embed], dim=-1)
        h = []
    ...
    def get_pred(self, x, cond, time, occ_grid, goal, collision, get_embedding=True, use_dropout=True, force_dropout=False):
        '''
            x : [ batch x horizon x transition ]
            returns : [batch x horizon]
        '''
