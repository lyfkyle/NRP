...
from utils import visualize_nodes_local
...
class Trainer(object):
    ...
    def load(self, checkpoint):
        ...
        '''
        loadpath = os.path.join(self.bucket, logger.prefix, f'checkpoint/state_{checkpoint}.pt')
    ...
    def render_samples(self, batch_size=5, n_samples=4):
        '''
            renders samples from (ema) diffusion model
        '''
        print("Rendering samples...")
        joint_bounds = self.dataset.joint_bounds.to(self.device)
        ...
        for i in range(batch_size):
            ...
            cond = batch[1]
            cond[0] = batch[1][0].repeat(n_samples, 1)
            conditions = to_device(cond, self.device)
            occ_grid = to_device(batch[2].repeat(n_samples, 1, 1, 1), self.device)
            goal_pos_normed = to_device(batch[3].repeat(n_samples, 1), self.device)
            free_prop = torch.ones_like(batch[4], device=self.device).repeat(n_samples, 1)
            samples = self.ema_model.conditional_sample(conditions, occ_grid, goal_pos_normed, free_prop)

            for j in range(n_samples):
                local_occ_grid = to_np(occ_grid[j, :]).reshape([40, 40])
                path = to_np(samples[j, :, :] * joint_bounds)
                start_pos = to_np(conditions[0][j, :].flatten() * joint_bounds)
                goal_pos = to_np(goal_pos_normed[j, :8].flatten() * joint_bounds)
                savepath = os.path.join(self.bucket, logger.prefix, 'checkpoint', "state_{}".format(self.step))
                os.makedirs(savepath, exist_ok=True)
                file_name = os.path.join(savepath, "env_{}_sample_{}.png".format(i, j))
