...
class BITStar:
    def __init__(self, maxIter=5, plot_flag=False, batch_size=100, sampling=None):
        # Hyperparameters
    ...
    def informed_sample(self, c_best, sample_num, vertices):
        ...
        while cur_num < sample_num:
            ...
            cur_num += 1
        # print("sample batch time:", time() - start_time)
        # print("sample success num:", len(sample_array))
    ...
    def is_point_free(self, point):
        ...
        # print("col checking time:", time() - start_time)
        return result
    ...
    def solve(self, maze, start, goal, allowed_time=float("inf"), max_samples=float("inf")):
        self.clear()
        self.env = maze
    ...
    def solve_step_expansion(self, maze, start, goal, max_extensions, step_size=50):
        self.clear()
        self.env = maze
        self.start = tuple(start)
        self.goal = tuple(goal)
    ...
    def plan(self, pathLengthLimit=INF, refine_time_budget=None):
        ...
        while True:
            ...
            if time.time() >= self.allowed_time:
                ...
                # print("Time: ", loop_start_time - self.start_time)
                break
            ...
            if not self.vertex_queue and not self.edge_queue:
                ...
                self.prune(c_best)
                # Problem is here: the extend on the first batch is taking 2s+ straight
                # samples = self.sampling(c_best, self.batch_size, self.vertices)
                # print("check1", time() - start_time_1)
                self.samples.extend(self.sampling(c_best, self.batch_size, self.vertices))
                # print("check2", time() - start_time_1)
        ...
        # return self.samples, self.edges, 0, success, self.T, time() - init_time
