...
class CNN_Encoder(nn.Module):
    def __init__(self, output_size, input_size=(1, 100, 100)):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(
                in_channels=1, out_channels=self.channel_mult * 1, kernel_size=4, stride=2, padding=1
            ),  # out = (16, 5, 5)
            nn.LeakyReLU(),
...
class CNN_Decoder(nn.Module):
    def __init__(self, embedding_size, input_size=(1, 10, 10)):
        ...
        self.fc = nn.Sequential(
            nn.Linear(self.input_dim, self.fc_output_dim), nn.BatchNorm1d(self.fc_output_dim), nn.LeakyReLU()
        )
...
class Encoder(nn.Module):
    def __init__(self, latent_size, context_size, input_size, feat_extractor_model_path=None):
        super().__init__()
...
class Decoder(nn.Module):
    def __init__(self, latent_size, context_size, out_size, feat_extractor_model_path=None):
        super().__init__()
...
class VAE(nn.Module):
    def __init__(self, latent_size, context_size, state_size, feat_extractor_model_path=None):
        super().__init__()
        self.latent_size = latent_size
        self.encoder = Encoder(latent_size, context_size, state_size, feat_extractor_model_path)
        self.decoder = Decoder(latent_size, context_size, state_size, feat_extractor_model_path)
