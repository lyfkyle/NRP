    ...
    for env_idx in range(env_num):
        for dataset_idx in range(dataset_num):
            ...
            for i, data in enumerate(datas):
                occ_grid, start_pos, goal_pos, path = data

                path = utils.interpolate(path)

                # utils.visualize_nodes_global(occ_grid, path, start_pos, goal_pos, show=False, save=True, file_name=os.path.join(CUR_DIR, "viz/gt_{}_{}.png".format(env_idx, dataset_idx)))

                for p in path:
                    all_pos_samples.append([occ_grid, start_pos, goal_pos, p])
...
if __name__ == '__main__':
    ...
    pos_samples = pos_samples[:1000000]

    # selection
    sel_pos_samples = pos_samples
    for sample in sel_pos_samples:
