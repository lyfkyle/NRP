...
def joints_from_names(p, body, names):
    return tuple(joint_from_name(p, body, name) for name in names)

def joint_from_name(p, body, name):
    for joint in get_joints(p, body):
        joint_name = get_joint_name(p, body, joint)
...
def get_joint_name(p, body, joint):
    return get_joint_info(p, body, joint).jointName  # .decode('UTF-8')

def get_link_name(p, body, link):
    if link == BASE_LINK:
        return get_base_name(p, body)
    return get_joint_info(p, body, link).linkName.decode('UTF-8')

def get_base_name(p, body):
    return get_body_info(p, body).base_name.decode(encoding='UTF-8')

def get_body_info(p, body):
    return BodyInfo(*p.getBodyInfo(body))

def get_body_name(p, body):
    return get_body_info(p, body).body_name.decode(encoding='UTF-8')