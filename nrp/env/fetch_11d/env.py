...
from nrp.env.fetch_11d import pb_ompl
...
class Fetch11DEnv:
    ...
    def __init__(self, gui=True, add_robot=True, load_floor=True):
        ...
        if add_robot:
            ...
            self.pb_ompl_interface = pb_ompl.PbOMPL(self.robot, self.obstacles, self.p)
        else:
            self.robot = None
    ...
    def get_link_positions(self, robot_state):
        # print(robot_state)
        return self.fk.get_link_positions(torch.tensor(robot_state).view(1, -1)).view(-1).numpy()
...
if __name__ == "__main__":
    ...
    with open(os.path.join(CUR_DIR, "../dataset/rls/occ_grid.npy"), "rb") as f:
        occ_grid = np.load(f)
    ...
    maze.load_mesh(osp.join(CUR_DIR, "../dataset/rls/rls_fixed.obj"))
