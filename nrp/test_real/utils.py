...
from planner.decomposed_planner import HybridAStar
...
def get_prm(env_dir):
    g = nx.read_graphml(osp.join(env_dir, "dense_g_rs_final.graphml"))
    return g


def get_mesh_path(env_dir):
    return osp.join(env_dir, "env_final.obj")
...
def calc_path_len(path, turning_radius=TURNING_RADIUS, arm_len_weight=0.125):
    ...
    for i, node in enumerate(path):
        if i == 0:
            continue
        length += calc_edge_len(path[i - 1], node, turning_radius, arm_len_weight).item()
...
def visualize_nodes_local(occ_g, cur_node_pos, start_pos, goal_pos, max_num=50, color_coding=False, show=True, save=False, file_name=None):
    from nrp.env.fetch_11d.env import Fetch11DEnv
...
def visualize_tree(mesh, occ_g, G, start_pos, goal_pos=None, cur_pos=None, target_pos=None, show=True, save=False, file_name=None, string=False):
    from nrp.env.fetch_11d.env import Fetch11DEnv
    maze = Fetch11DEnv(gui=False, add_robot=False)
    if mesh is not None:
        maze.load_mesh(mesh)
...
def visualize_tree_simple(occ_g, G, start_pos, goal_pos, show=True, save=False, file_name=None, string=False):
...
def visualize_robot(robot_state, start=False, goal=False, mu=False, ax=None, s=15):
    ...
    if start:
        plt.scatter(base_x, base_y, color="yellow", marker='s', s=base_marker_size, alpha=1)  # init
    elif goal:
        plt.scatter(base_x, base_y, color="red", marker='s', s=base_marker_size, alpha=1)  # init
    else:
...
def is_robot_within_local_env(robot_state, env_size):
    ...
    # Dont know how to calculate exactly in 3d. Use this approximation
    if math.fabs(base_x) > env_size - 0.5 or math.fabs(base_y) > env_size - 0.5:
        return False
...
class FkTorch():
    def __init__(self, device):
        ...
        chain = pk.build_serial_chain_from_urdf(open(osp.join(CUR_DIR, "../../robot_model/fetch.urdf")).read(),
                                                "gripper_link")
    ...
    def get_link_positions(self, robot_state):
        ...
        linkpos = torch.cat((torso_pos, shoulder_pan_pos, shoulder_lift_pos, upperarm_roll_pos, elbow_flex_pos,
                             forearm_roll_pos, wrist_flex_pos, wrist_roll_pos), dim=1).view(bs, -1, 3)
        # print(linkpos)
...
# def is_edge_free(maze, node1_state, node2_state, step_size=0.2, num_points_per_edge = None):
#     node1_pos = np.array(node1_state)
#     node2_pos = np.array(node2_state)
...
#     if np.max(np.abs(diff)) < step_size:
#         return True
...
#     for i in range(num_points_per_edge):
#         nodepos = node1_pos + step_size * i
#         if not maze.pb_ompl_interface.is_state_valid(nodepos.tolist()):
#             return False
#     return True

# def interpolate(path, step_size=0.2, num_points_per_edge = None):
#     new_path = []
#     for i in range(1, len(path)):
#         cur_p = np.array(path[i])
...
#         step_size = diff / (num_points_per_edge - 1) # actual step_size, nparray
#         # assert(np.max(step) < 0.1 and np.min(step) > -0.1)

#         for j in range(num_points_per_edge - 1):
#             nodepos = prev_p + step_size * j
#             new_path.append(nodepos.tolist())
...
def get_ebsa_path(occ_grid, start_pos, target_pos):
    ...
    if astar_path is not None:
        ...
        for i in range(len(astar_path[0])):
            expert_base_path.append([astar_path[0][i] * occ_grid_resolution, astar_path[1][i] * occ_grid_resolution, astar_path[2][i]])
