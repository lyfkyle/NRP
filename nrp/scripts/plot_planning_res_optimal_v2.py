...
planners = ["nrp_d_star", "nrp_g_star", "cvae_star", "next", "rrt_star", "bit_star"]
legends = ["NRP*-d", "NRP*-g", "CVAE-IRRT*", "NEXT", "IRRT*", "BIT*"]

fig, (ax1, ax2) = plt.subplots(nrows=1, ncols=2, figsize=(12, 5))  # unpack

num_extensions = np.arange(0, max_num_extensions + 1, 25)

for i, planner in enumerate(planners):
    if not osp.exists(osp.join(ROOT_DIR, f"test/eval_res/snake_8d/test_time/{planner}")):
        continue
...
ax1.set_xticks(np.arange(0, max_num_extensions + 1, 50))
ax1.set_xticks(np.arange(0, max_num_extensions + 1, 25), minor=True)  # set minor ticks on x-axis
...
for i, planner in enumerate(planners):
    if not osp.exists(osp.join(ROOT_DIR, f"test/eval_res/{args.env}/test_time/{planner}")):
        continue

    planner_success = []
    for repeat in range(args.repeat):
        with open(
            osp.join(ROOT_DIR, "test/eval_res/{}/test_time/{}/result_{}.json".format(args.env, planner, repeat)), "r"
        ) as f:
            planner_success_tmp = json.load(f)
...
plt.savefig(f"planning_res_optimal_v2_{args.env}.png", bbox_inches="tight")
