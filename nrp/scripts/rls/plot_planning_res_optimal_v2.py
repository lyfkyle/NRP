...
c_map = {
    ...
    "RRT-IS": "tab:olive",
    "Decoupled": "tab:brown",
    "Decoupled-10s": "tab:purple",
    "Decoupled-IS": "tab:purple",
    "Decoupled-RRT-IS-empty": "tab:olive",
    "Decoupled-empty": "tab:olive",
    "FIRE": "tab:purple",
    "VQMPT": "tab:green",
}
...
time_planners = ["neural_d_star", "neural_g_star", "rrt_star", "bit_star", "decoupled_rrt_star", "decoupled_rrt_star_10s"]
legends = ["NRP*-d", "NRP*-g", "IRRT*", "BIT*", "Decoupled", "Decoupled-10s"]
...
for i, planner in enumerate(ext_planners):
    ...
    p = ax1.plot(num_extensions, planner_success_mean, 'o-', label=legends[i], color=c_map[legends[i]])
...
for i, planner in enumerate(time_planners):
    planner_success = []
    for repeat in range(10):
    ...
    p = ax2.plot(planning_time, planner_success_mean, 'o-', label=legends[i], color=c_map[legends[i]])
...
ax2.legend(loc='lower center', ncols=len(time_planners), bbox_to_anchor=(-0.1, -0.22), fancybox=True, shadow=True)
...
plt.savefig(osp.join(CUR_DIR, "planning_res_optimal_v3_decoupled.png"), bbox_inches="tight")
