...
def convert_data(env_idx, train_env_dirs, input_data_parent_dir, output_data_parent_dir, data_cnt, lock):
    ...
    for file in dir_list:
        ...
        optimal_data = []
        optimal_data.append([local_occ_grid, local_start_pos, local_goal_pos, local_expert_wp, env_idx, start_pos])
        for node in local_prm.nodes():
            node_pos = utils.node_to_numpy(local_prm, node)
            ...
            if optimal:
                optimal_data.append(
                    [local_occ_grid, local_start_pos, local_goal_pos, local_node_pos, env_idx, start_pos]
                )

        print(f"num of near-optimal data: {len(optimal_data)}")

        # test visualize
        # if local_env_idx < 1:
        #     near_optimal_wps = [utils.local_to_global(x[3], start_pos) for x in optimal_data[:5]]
        #     utils.visualize_nodes_global(mesh_path, occ_grid, near_optimal_wps, start_pos, goal_pos, sample_pos=expert_path[1], show=False, save=True, file_name=os.path.join(viz_data_dir, f"data_{env_idx}_{local_env_idx}_{goal_idx}.png"))
        # else:
        #     break

        # Add optimal samples into dataset
        lock.acquire()
        for data in optimal_data[:5]:
...
if __name__ == "__main__":
    # v2: PRM with resample
    # v3: add near-optimal dataset to nrp_g training data
    ...
    output_parent_data_dir = osp.join(CUR_DIR, f"dataset/train_01_critical_v2_out_g")
    viz_data_dir = osp.join(CUR_DIR, f"dataset/train_01_critical_v2_out_g_viz")
    local_env_size = 2.0
