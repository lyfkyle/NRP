...
def process_env(env_dirs, env_idx):
    ...
    dense_G.add_nodes_from(
        [
            ("n{}".format(i), {"coords": ",".join(map(str, state)), "col": not col_status[i]})
            for i, state in enumerate(states)
        ]
    )

    free_node_poss = np.array(
        [state_to_numpy(dense_G.nodes[node]["coords"]) for node in dense_G.nodes() if not dense_G.nodes[node]["col"]]
    )
    ...
    utils.visualize_tree_simple(
        occ_grid, dense_G, None, None, show=False, save=True, file_name=osp.join(env_dir, f"tree.png")
    )
